...
func TestStore_GetTotalSpending(t *testing.T) {
	s, err := initDB()
	assert.NoError(t, err)
	res, err := s.GetTotalSpending(context.Background())
	assert.NoError(t, err)
	assert.NotNil(t, res)
	assert.NoError(t, s.Close())
}

func TestStore_AddTotalSpending(t *testing.T) {
	s, err := initDB()
	assert.NoError(t, err)
	assert.NoError(t, s.AddTotalSpending(context.Background(), &Spending{
		Amount:                  10,
		Chirp3HDCharacters:      10,
		StandardVoiceCharacters: 10,
		GeminiInputTokens:       10,
